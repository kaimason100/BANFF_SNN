...
// snn_regress_time_loop_gpu_mex.cu
//
// Dataset-GPU inner loop for **regression** task with adLIF neurons + 2-stage synapses.
...
// - Advances e-prop eligibilities per neuron and returns the SUM over
//   the averaging window of Ebar_f (N_hidden x 1) and SUM of r_read ("phi_sum_win").
//
// Command interface from MATLAB:
//
//   snn_regress_time_loop_gpu_mex('init', W_in, W_out_base_rec,
//       W_out, Eta_rec, dself, B, W_read, scalar parameters..., dimensions...);
//   snn_regress_time_loop_gpu_mex('update_params', B, W_read);
//   [Z_avg, Y_read_avg, Ebar_f_sum_win, phi_sum_win] =
//       snn_regress_time_loop_gpu_mex('run_batch', X_batch);
//   [Z_avg, Y_read_avg] =
//       snn_regress_time_loop_gpu_mex('run_batch_outputs', X_batch);
//   snn_regress_time_loop_gpu_mex('clear');
//
// Build:
//   Run Regression/build/compile_regression_gpu_mex.mlx
//
// Fast-mode notes:
...
// - run_batch loops over samples inside one MEX call to reduce MATLAB overhead.
//
