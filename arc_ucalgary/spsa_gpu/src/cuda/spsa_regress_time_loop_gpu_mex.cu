...
// spsa_regress_time_loop_gpu_mex.cu
//
// Copied regression GPU inner loop for SPSA loss probes with adLIF neurons and
// two-stage synapses.
// - Constant input vector is presented for 'steps' time steps.
// - Computes only average-window outputs:
//       Z_avg      = mean(W_out  * r(t_k))
//       Y_read_avg = mean(W_read * r_read(t_k))
// - The SPSA MATLAB wrapper normally calls update_bias plus validate_primary/predict_primary.
// - Legacy gradient/readout training commands remain available in this copied source but
//   are not the main SPSA training path.
//
// Command interface from MATLAB:
//
//   spsa_regress_time_loop_gpu_mex('init', W_in, W_out_base_rec,
//       W_out, Eta_rec, dself, B, W_read, scalar parameters..., dimensions...);
//   spsa_regress_time_loop_gpu_mex('update_bias', B);
//   [loss,r,count] =
//       spsa_regress_time_loop_gpu_mex('validate_primary', 'val', batch_size);
//   [loss,r,count,Z] =
//       spsa_regress_time_loop_gpu_mex('predict_primary', 'test', batch_size);
//   spsa_regress_time_loop_gpu_mex('clear');
//
// Build:
//   Run spsa_gpu/build/compile_spsa_gpu_mex.m
//
// Fast-mode notes:
// - Keeps static weights and all GPU buffers persistent across samples/batches.
...
// - Dataset commands loop over samples inside one MEX call to reduce MATLAB overhead.
//
...
#define CUDA_CHECK(call) do { \
    ...
    if (err_ != cudaSuccess) { \
        mexErrMsgIdAndTxt("snn_regress_mex:cuda", "CUDA error %d (%s) at %s:%d", \
...
static inline void assertSingle(const mxArray* a, const char* name)
{
    if (mxGetClassID(a) != mxSINGLE_CLASS || mxIsComplex(a))
        mexErrMsgIdAndTxt("snn_regress_mex:type", "%s must be real(single)", name);
}
...
// Two-stage synaptic cascades with fractional durations (z_const=0 here)
                                   ...
                                   int N)
{
    ...
    float x1 = gsr_f * x_syn[i];     // z_const = 0
    x_syn[i] = x1;
...
__global__ void kRegressLossGradStats(const float* __restrict__ Z,
                                      ...
                                      float* __restrict__ statsNet,
                                      float* __restrict__ statsRead,
                                      int B)
{
    ...
    gZnet[b] = dn;
    gZread[b] = dr;
    lossNet[b] = 0.5f * dn * dn;
    lossRead[b] = 0.5f * dr * dr;
    statsNet[0 + 5*b] = zn;
    statsNet[1 + 5*b] = zn * zn;
    statsNet[2 + 5*b] = zn * y;
    statsNet[3 + 5*b] = y;
    statsNet[4 + 5*b] = y * y;
    statsRead[0 + 5*b] = zr;
    statsRead[1 + 5*b] = zr * zr;
    statsRead[2 + 5*b] = zr * y;
    statsRead[3 + 5*b] = y;
    statsRead[4 + 5*b] = y * y;
}
...
__global__ void kReduceRegressMetrics(const float* __restrict__ lossNet,
                                      const float* __restrict__ lossRead,
                                      const float* __restrict__ statsNet,
                                      const float* __restrict__ statsRead,
                                      ...
                                      int B)
{
    ...
    for (int b=tid; b<B; b+=blockDim.x) {
        ...
        v[1] += lossRead[b];
        for (int k=0; k<5; ++k) {
            v[2+k] += statsNet[k + 5*b];
            v[7+k] += statsRead[k + 5*b];
        }
    }
    for (int k=0; k<12; ++k) sh[k][tid] = v[k];
    __syncthreads();
    for (int s=blockDim.x>>1; s>0; s>>=1) {
        if (tid < s) {
            for (int k=0; k<12; ++k) sh[k][tid] += sh[k][tid+s];
        }
        __syncthreads();
    }
    if (tid == 0) {
        for (int k=0; k<12; ++k) atomicAdd(metricSums + k, sh[k][0]);
    }
}
...
{
    assertSingle(a, nm);
    if (mxGetNumberOfElements(a) != 1) mexErrMsgIdAndTxt("snn_regress_gpu_mex:scalar", "%s must be scalar", nm);
...
{
    if (!G.initialized) mexErrMsgIdAndTxt("snn_regress_gpu_mex:state", "Call snn_regress_time_loop_gpu_mex('init', ...) first.");
}
static void cublasCheck(cublasStatus_t st, const char* what)
{
    if (st != CUBLAS_STATUS_SUCCESS) mexErrMsgIdAndTxt("snn_regress_gpu_mex:cublas", "cuBLAS error %d in %s", (int)st, what);
}
static void allocContext()
{
    ...
    CUDA_CHECK(cudaMalloc(&G.d_metricSums,sizeof(float)*12));
            ...
            if (G.recurrent_storage == REC_STORAGE_DENSE) {
                ...
                if (G.rec_nnz > 0) {
                    ...
                    if (edgeTotal64 > (long long)INT_MAX)
                        mexErrMsgIdAndTxt("snn_regress_gpu_mex:sparseOverflow", "Sparse recurrent edge batch exceeds int32 CUDA indexing; reduce p_rec, N_hidden or batch_size.");
                ...
                mexErrMsgIdAndTxt("snn_regress_gpu_mex:arch", "Unknown recurrent_storage_id %d.", G.recurrent_storage);
            }
            kBuildItotFullRankBatch<<<gN,t>>>(G.d_I_in,G.d_Irec_scaled,G.d_B,G.d_I_rec,G.d_I_tot,N,total); CUDA_CHECK(cudaGetLastError());
        } else {
            mexErrMsgIdAndTxt("snn_regress_gpu_mex:arch", "Unknown recurrent_mode_id %d.", G.recurrent_mode);
        }
...
static void gatherBatch(const float* X, int nX, const float* Y, int nY, const int* order, int offset, int B, std::vector<float>& hx, std::vector<float>& hy)
{
    ...
    for (int j=0;j<B;++j) {
        ...
        if (idx < 0 || idx >= nX || idx >= nY) mexErrMsgIdAndTxt("snn_regress_gpu_mex:order","Order index out of range");
...
{
    kRegressLossGradStats<<<(B+255)/256,256>>>(G.d_Z_sum,G.d_Yread_sum,G.d_Y,G.d_gZnet,G.d_gZread,G.d_lossNet,G.d_lossRead,G.d_statsNet,G.d_statsRead,B);
    CUDA_CHECK(cudaGetLastError());
}
static float corrFromStats(double sx, double sx2, double sxy, double sy, double sy2, double n)
{
    double num = sxy - sx*sy/n;
    double vx = sx2 - sx*sx/n;
    double vy = sy2 - sy*sy/n;
    double den = sqrt(fmax(vx,0.0) * fmax(vy,0.0));
    if (!isfinite(den) || den <= 2.220446049250313e-16) return NAN;
...
{
    if (!mxIsChar(a)) mexErrMsgIdAndTxt("snn_regress_gpu_mex:type", "%s must be a char/string", nm);
    char* c = mxArrayToString(a);
    if (!c) mexErrMsgIdAndTxt("snn_regress_gpu_mex:type", "Could not parse %s", nm);
    std::string s(c);
    mxFree(c);
    return s;
}
...
static GpuData& selectData(const std::string& split)
{
    ...
    mexErrMsgIdAndTxt("snn_regress_gpu_mex:split", "Unknown split '%s'", split.c_str());
    return G.train;
}
...
static void validateOrderVector(const mxArray* order_m, int n)
{
    if (mxGetClassID(order_m) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("snn_regress_gpu_mex:type", "order must be int32");
    if ((int)mxGetNumberOfElements(order_m) != n)
        mexErrMsgIdAndTxt("snn_regress_gpu_mex:shape", "order length mismatch");
    const int* order = (const int*)mxGetData(order_m);
    for (int i = 0; i < n; ++i) {
        if (order[i] < 1 || order[i] > n)
            mexErrMsgIdAndTxt("snn_regress_gpu_mex:order", "Order index out of range");
    }
}
...
{
    (void)nlhs; (void)plhs;
    if (nrhs != 5) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args", "set_data expects split, X, Y, precompute_inputs");
    requireInitialized();
    ...
    assertSingle(X_m, "X");
    assertSingle(Y_m, "Y");
    if ((int)mxGetM(X_m) != G.N_in) mexErrMsgIdAndTxt("snn_regress_gpu_mex:shape", "X must be [N_in x samples]");
    if ((int)mxGetM(Y_m) != G.N_out) mexErrMsgIdAndTxt("snn_regress_gpu_mex:shape", "Y must be [N_out x samples]");
    int n = (int)mxGetN(X_m);
    if ((int)mxGetN(Y_m) != n) mexErrMsgIdAndTxt("snn_regress_gpu_mex:shape", "X/Y sample count mismatch");
...
static void cmdClearData(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    if (nrhs != 2) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args", "clear_data expects zero or one split name");
...
{
    (void)nlhs; (void)plhs;
    if (nrhs != 9) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args", "init_optim expects bias_b1,bias_b2,bias_eps,read_lr,read_b1,read_b2,read_eps,read_weight_decay");
    requireInitialized();
...
{
    if (!G.optim_initialized) mexErrMsgIdAndTxt("snn_regress_gpu_mex:optim", "Call init_optim before train_epoch_gpu.");
...
static void runGpuDataset(GpuData& D, const mxArray* order_m, int batchSize, bool doGrad, bool updateWeights,
                               ...
                               double& lossN, double& lossR, double& rN, double& rR, int& count)
{
    if (!D.has) mexErrMsgIdAndTxt("snn_regress_gpu_mex:data", "Requested dataset split has not been loaded with set_data.");
    if (batchSize < 1 || batchSize > G.max_batch) mexErrMsgIdAndTxt("snn_regress_gpu_mex:shape", "batch_size exceeds max_batch from init");
    ...
    CUDA_CHECK(cudaMemset(G.d_metricSums,0,sizeof(float)*12));
    for (int off=0; off<D.n; off+=batchSize) {
        ...
        kReduceRegressMetrics<<<1,256>>>(G.d_lossNet,G.d_lossRead,G.d_statsNet,G.d_statsRead,G.d_metricSums,B);
        CUDA_CHECK(cudaGetLastError());
    }
    float h[12] = {0};
    CUDA_CHECK(cudaMemcpy(h, G.d_metricSums, sizeof(float)*12, cudaMemcpyDeviceToHost));
    lossN = h[0]; lossR = h[1];
    rN = corrFromStats(h[2],h[3],h[4],h[5],h[6],(double)D.n);
    rR = corrFromStats(h[7],h[8],h[9],h[10],h[11],(double)D.n);
    count = D.n;
...
{
    if (nrhs != 43) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args","init expects 42 arguments after command string");
    ...
    if (G.recurrent_mode != RECURRENT_LOW_RANK && G.recurrent_mode != RECURRENT_FULL_RANK)
        mexErrMsgIdAndTxt("snn_regress_gpu_mex:arch","recurrent_mode_id must be 0 (low_rank) or 1 (full_rank)");
    if (G.decoder_mode != DECODER_SHARED && G.decoder_mode != DECODER_SIGNED)
        mexErrMsgIdAndTxt("snn_regress_gpu_mex:arch","decoder_mode_id must be 0 (shared) or 1 (signed)");
    if (G.recurrent_storage != REC_STORAGE_DENSE && G.recurrent_storage != REC_STORAGE_SPARSE)
        mexErrMsgIdAndTxt("snn_regress_gpu_mex:arch","recurrent_storage_id must be 0 (dense) or 1 (sparse)");
    ...
    if (mxGetClassID(rec_post_m) != mxINT32_CLASS || mxGetClassID(rec_pre_m) != mxINT32_CLASS)
        mexErrMsgIdAndTxt("snn_regress_gpu_mex:type","rec_post_idx and rec_pre_idx must be int32");
    if ((int)mxGetM(W_in_m)!=G.N_hidden || (int)mxGetN(W_in_m)!=G.N_in) mexErrMsgIdAndTxt("snn_regress_gpu_mex:shape","W_in shape mismatch");
    if ((int)mxGetM(W_ob_m)!=G.N_rec || (int)mxGetN(W_ob_m)!=G.N_hidden) mexErrMsgIdAndTxt("snn_regress_gpu_mex:shape","W_out_base_rec shape mismatch");
    if ((int)mxGetM(W_out_m)!=G.N_out || (int)mxGetN(W_out_m)!=G.N_hidden) mexErrMsgIdAndTxt("snn_regress_gpu_mex:shape","W_out shape mismatch");
    if ((int)mxGetM(Eta_m)!=G.N_hidden || (int)mxGetN(Eta_m)!=G.N_rec) mexErrMsgIdAndTxt("snn_regress_gpu_mex:shape","Eta_rec shape mismatch");
    if ((int)mxGetNumberOfElements(dself_m)!=G.N_hidden || (int)mxGetNumberOfElements(B_m)!=G.N_hidden) mexErrMsgIdAndTxt("snn_regress_gpu_mex:shape","bias vector size mismatch");
    if ((int)mxGetM(W_read_m)!=G.N_out || (int)mxGetN(W_read_m)!=G.N_hidden) mexErrMsgIdAndTxt("snn_regress_gpu_mex:shape","W_read shape mismatch");
    if (G.rec_nnz < 0 || (int)mxGetNumberOfElements(rec_post_m) != G.rec_nnz || (int)mxGetNumberOfElements(rec_pre_m) != G.rec_nnz || (int)mxGetNumberOfElements(rec_w_m) != G.rec_nnz)
        mexErrMsgIdAndTxt("snn_regress_gpu_mex:shape","sparse recurrent edge-list length mismatch");
    if (G.recurrent_mode == RECURRENT_FULL_RANK && G.recurrent_storage == REC_STORAGE_DENSE && ((int)mxGetM(W_rec_m)!=G.N_hidden || (int)mxGetN(W_rec_m)!=G.N_hidden))
        mexErrMsgIdAndTxt("snn_regress_gpu_mex:shape","W_rec shape mismatch for full_rank recurrence");
    if (G.max_batch < 1) mexErrMsgIdAndTxt("snn_regress_gpu_mex:shape","max_batch must be positive");
    allocContext();
...
{
    (void)nlhs; (void)plhs; if (nrhs!=3) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args","update_params expects B and W_read"); requireInitialized(); assertSingle(prhs[1],"B"); assertSingle(prhs[2],"W_read");
    if ((int)mxGetNumberOfElements(prhs[1]) != G.N_hidden || (int)mxGetM(prhs[2]) != G.N_out || (int)mxGetN(prhs[2]) != G.N_hidden)
        mexErrMsgIdAndTxt("snn_regress_gpu_mex:shape","B or W_read shape mismatch");
...
static void runDataset(const mxArray* X_m, const mxArray* Y_m, const mxArray* order_m, int batchSize, bool doGrad,
                       double& lossN, double& lossR, double& rN, double& rR, int& count)
{
    assertSingle(X_m,"X"); assertSingle(Y_m,"Y"); if ((int)mxGetM(X_m)!=G.N_in || (int)mxGetM(Y_m)!=G.N_out) mexErrMsgIdAndTxt("snn_regress_gpu_mex:shape","X/Y must be transposed [features/outputs x samples]");
    int n=(int)mxGetN(X_m); if ((int)mxGetN(Y_m)!=n) mexErrMsgIdAndTxt("snn_regress_gpu_mex:shape","X/Y sample count mismatch"); if (batchSize<1 || batchSize>G.max_batch) mexErrMsgIdAndTxt("snn_regress_gpu_mex:shape","batch_size exceeds max_batch from init");
    const float* X=(const float*)mxGetData(X_m); const float* Y=(const float*)mxGetData(Y_m); const int* order=nullptr; if(order_m && !mxIsEmpty(order_m)){ if(mxGetClassID(order_m)!=mxINT32_CLASS) mexErrMsgIdAndTxt("snn_regress_gpu_mex:type","order must be int32"); order=(const int*)mxGetData(order_m); }
    ...
    std::vector<float> hx, hy, hLossN(G.max_batch), hLossR(G.max_batch), hStatsN(5*G.max_batch), hStatsR(5*G.max_batch);
    lossN=0; lossR=0; double sN[5]={0,0,0,0,0}, sR[5]={0,0,0,0,0}; count=n;
    for(int off=0; off<n; off+=batchSize){ int B=std::min(batchSize,n-off); gatherBatch(X,n,Y,n,order,off,B,hx,hy); CUDA_CHECK(cudaMemcpy(G.d_X,hx.data(),sizeof(float)*G.N_in*B,cudaMemcpyHostToDevice)); CUDA_CHECK(cudaMemcpy(G.d_Y,hy.data(),sizeof(float)*G.N_out*B,cudaMemcpyHostToDevice)); runBatchDynamics(B); computeLossGrad(B); if(doGrad) accumulateGradients(B); CUDA_CHECK(cudaMemcpy(hLossN.data(),G.d_lossNet,sizeof(float)*B,cudaMemcpyDeviceToHost)); CUDA_CHECK(cudaMemcpy(hLossR.data(),G.d_lossRead,sizeof(float)*B,cudaMemcpyDeviceToHost)); CUDA_CHECK(cudaMemcpy(hStatsN.data(),G.d_statsNet,sizeof(float)*5*B,cudaMemcpyDeviceToHost)); CUDA_CHECK(cudaMemcpy(hStatsR.data(),G.d_statsRead,sizeof(float)*5*B,cudaMemcpyDeviceToHost)); for(int j=0;j<B;++j){lossN+=hLossN[j]; lossR+=hLossR[j]; for(int k=0;k<5;++k){sN[k]+=hStatsN[k+5*j]; sR[k]+=hStatsR[k+5*j];} } }
    rN=corrFromStats(sN[0],sN[1],sN[2],sN[3],sN[4],(double)n); rR=corrFromStats(sR[0],sR[1],sR[2],sR[3],sR[4],(double)n);
}
static void cmdTrainEpoch(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    if(nrhs!=5 || nlhs!=7) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args","train_epoch returns [lossN,lossR,rN,rR,count,gB,Grad]"); requireInitialized(); int batchSize=getIntScalar(prhs[4],"batch_size"); double ln,lr,rn,rr; int count; runDataset(prhs[1],prhs[2],prhs[3],batchSize,true,ln,lr,rn,rr,count); plhs[0]=mxCreateDoubleScalar(ln); plhs[1]=mxCreateDoubleScalar(lr); plhs[2]=mxCreateDoubleScalar(rn); plhs[3]=mxCreateDoubleScalar(rr); plhs[4]=mxCreateDoubleScalar((double)count); plhs[5]=mxCreateNumericMatrix(G.N_hidden,1,mxSINGLE_CLASS,mxREAL); plhs[6]=mxCreateNumericMatrix(G.N_out,G.N_hidden,mxSINGLE_CLASS,mxREAL); CUDA_CHECK(cudaMemcpy(mxGetData(plhs[5]),G.d_gB_epoch,sizeof(float)*G.N_hidden,cudaMemcpyDeviceToHost)); CUDA_CHECK(cudaMemcpy(mxGetData(plhs[6]),G.d_Grad_epoch,sizeof(float)*G.N_out*G.N_hidden,cudaMemcpyDeviceToHost));
}
static void cmdValidate(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    if(nrhs!=4 || nlhs!=5) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args","validate returns [lossN,lossR,rN,rR,count]"); requireInitialized(); int batchSize=getIntScalar(prhs[3],"batch_size"); double ln,lr,rn,rr; int count; runDataset(prhs[1],prhs[2],nullptr,batchSize,false,ln,lr,rn,rr,count); plhs[0]=mxCreateDoubleScalar(ln); plhs[1]=mxCreateDoubleScalar(lr); plhs[2]=mxCreateDoubleScalar(rn); plhs[3]=mxCreateDoubleScalar(rr); plhs[4]=mxCreateDoubleScalar((double)count);
}


static void cmdPredictGpu(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    if (nrhs != 3 || nlhs != 7) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args", "predict_gpu returns [lossN,lossR,rN,rR,count,Z,Yread]");
    requireInitialized();
    GpuData& D = selectData(getString(prhs[1], "split"));
    if (!D.has) mexErrMsgIdAndTxt("snn_regress_gpu_mex:data", "Requested dataset split has not been loaded with set_data.");
    ...
    mxArray* Yall = mxCreateNumericMatrix(G.N_out, D.n, mxSINGLE_CLASS, mxREAL);
    float* hZ = (float*)mxGetData(Zall);
    float* hY = (float*)mxGetData(Yall);
    CUDA_CHECK(cudaMemset(G.d_metricSums,0,sizeof(float)*12));
    for (int off=0; off<D.n; off+=batchSize) {
        int B = std::min(batchSize, D.n-off);
        int xRows = D.precomputed ? G.N_hidden : G.N_in;
        ...
        CUDA_CHECK(cudaMemcpy(hY + (size_t)G.N_out*off, G.d_Yread_sum, sizeof(float)*G.N_out*B, cudaMemcpyDeviceToHost));
    }
    float h[12] = {0};
    CUDA_CHECK(cudaMemcpy(h, G.d_metricSums, sizeof(float)*12, cudaMemcpyDeviceToHost));
    plhs[0]=mxCreateDoubleScalar(h[0]);
    plhs[1]=mxCreateDoubleScalar(h[1]);
    plhs[2]=mxCreateDoubleScalar(corrFromStats(h[2],h[3],h[4],h[5],h[6],(double)D.n));
    plhs[3]=mxCreateDoubleScalar(corrFromStats(h[7],h[8],h[9],h[10],h[11],(double)D.n));
    plhs[4]=mxCreateDoubleScalar((double)D.n);
    plhs[5]=Zall;
    plhs[6]=Yall;
}
...
{
    if (nrhs != 5 || nlhs > 5) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args", "train_epoch_gpu returns up to [lossN,lossR,rN,rR,count]");
    requireInitialized();
    GpuData& D = selectData(getString(prhs[1], "split"));
    int batchSize = getIntScalar(prhs[3], "batch_size");
    float lrBias = getScalarSingle(prhs[4], "lr_bias");
    double ln, lr, rn, rr; int count;
    runGpuDataset(D, prhs[2], batchSize, true, true, lrBias, true, true, ln, lr, rn, rr, count);
    if (nlhs > 0) plhs[0]=mxCreateDoubleScalar(ln);
    if (nlhs > 1) plhs[1]=mxCreateDoubleScalar(lr);
    if (nlhs > 2) plhs[2]=mxCreateDoubleScalar(rn);
    if (nlhs > 3) plhs[3]=mxCreateDoubleScalar(rr);
...
{
    if (nrhs != 5 || nlhs > 3) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args", "train_primary_epoch returns up to [loss,r,count]");
    requireInitialized();
    GpuData& D = selectData(getString(prhs[1], "split"));
    int batchSize = getIntScalar(prhs[3], "batch_size");
    float lrBias = getScalarSingle(prhs[4], "lr_bias");
    double lossN, lossIgnored, rN, rIgnored; int count;
    runGpuDataset(D, prhs[2], batchSize, true, true, lrBias, true, false, lossN, lossIgnored, rN, rIgnored, count);
    if (nlhs > 0) plhs[0]=mxCreateDoubleScalar(lossN);
    if (nlhs > 1) plhs[1]=mxCreateDoubleScalar(rN);
...
{
    if (nrhs != 3 || nlhs > 5) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args", "validate_gpu returns up to [lossN,lossR,rN,rR,count]");
    requireInitialized();
    GpuData& D = selectData(getString(prhs[1], "split"));
    int batchSize = getIntScalar(prhs[2], "batch_size");
    double ln, lr, rn, rr; int count;
    runGpuDataset(D, nullptr, batchSize, false, false, 0.f, false, false, ln, lr, rn, rr, count);
    if (nlhs > 0) plhs[0]=mxCreateDoubleScalar(ln);
    if (nlhs > 1) plhs[1]=mxCreateDoubleScalar(lr);
    if (nlhs > 2) plhs[2]=mxCreateDoubleScalar(rn);
    if (nlhs > 3) plhs[3]=mxCreateDoubleScalar(rr);
...
{
    if (nrhs != 3 || nlhs > 3) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args", "validate_primary returns up to [loss,r,count]");
    requireInitialized();
    GpuData& D = selectData(getString(prhs[1], "split"));
    int batchSize = getIntScalar(prhs[2], "batch_size");
    double lossN, lossIgnored, rN, rIgnored; int count;
    runGpuDataset(D, nullptr, batchSize, false, false, 0.f, false, false, lossN, lossIgnored, rN, rIgnored, count);
    if (nlhs > 0) plhs[0]=mxCreateDoubleScalar(lossN);
    if (nlhs > 1) plhs[1]=mxCreateDoubleScalar(rN);
    if (nlhs > 2) plhs[2]=mxCreateDoubleScalar((double)count);
}
...
{
    if (nrhs != 3 || nlhs != 4) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args", "predict_primary returns [loss,r,count,Z]");
    requireInitialized();
    GpuData& D = selectData(getString(prhs[1], "split"));
    if (!D.has) mexErrMsgIdAndTxt("snn_regress_gpu_mex:data", "Requested dataset split has not been loaded with set_data.");
    int batchSize = getIntScalar(prhs[2], "batch_size");
    ...
    CUDA_CHECK(cudaMemcpy(h, G.d_metricSums, sizeof(float)*12, cudaMemcpyDeviceToHost));
    plhs[0]=mxCreateDoubleScalar(h[0]);
    plhs[1]=mxCreateDoubleScalar(corrFromStats(h[2],h[3],h[4],h[5],h[6],(double)D.n));
...
{
    (void)prhs;
    if (nrhs != 1 || nlhs != 2) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args", "get_params returns [B,W_read]");
    requireInitialized();
...
{
    (void)prhs;
    if (nrhs != 1 || nlhs != 1) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args", "get_bias returns B");
    requireInitialized();
    plhs[0]=mxCreateNumericMatrix(G.N_hidden,1,mxSINGLE_CLASS,mxREAL);
    CUDA_CHECK(cudaMemcpy(mxGetData(plhs[0]), G.d_B, sizeof(float)*G.N_hidden, cudaMemcpyDeviceToHost));
}
...
{
    (void)prhs;
    if (nrhs != 1 || nlhs != 4) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args", "get_optim_state returns [m_b,v_b,vhat_b,t_adam]");
    requireInitialized();
    if (!G.optim_initialized) mexErrMsgIdAndTxt("snn_regress_gpu_mex:optim", "Call init_optim before get_optim_state.");
    plhs[0]=mxCreateNumericMatrix(G.N_hidden,1,mxSINGLE_CLASS,mxREAL);
...
{
    (void)nlhs; (void)plhs;
    if (nrhs != 5) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args", "set_optim_state expects m_b,v_b,vhat_b,t_adam");
    requireInitialized();
    if (!G.optim_initialized) mexErrMsgIdAndTxt("snn_regress_gpu_mex:optim", "Call init_optim before set_optim_state.");
    ...
    if ((int)mxGetNumberOfElements(prhs[1]) != G.N_hidden ||
        ...
        mexErrMsgIdAndTxt("snn_regress_gpu_mex:shape", "Optimizer state vector length mismatch");
...
{
    (void)nlhs; (void)plhs;
    if (nrhs != 2) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args", "update_bias expects B");
    requireInitialized();
    assertSingle(prhs[1], "B");
    if ((int)mxGetNumberOfElements(prhs[1]) != G.N_hidden)
        mexErrMsgIdAndTxt("snn_regress_gpu_mex:shape", "B shape mismatch");
...
{
    if (nrhs != 4 || nlhs > 5) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args", "train_read_epoch_gpu returns up to [lossN,lossR,rN,rR,count]");
    requireInitialized();
    GpuData& D = selectData(getString(prhs[1], "split"));
    int batchSize = getIntScalar(prhs[3], "batch_size");
    double ln, lr, rn, rr; int count;
    runGpuDataset(D, prhs[2], batchSize, true, true, 0.f, false, true, ln, lr, rn, rr, count);
    if (nlhs > 0) plhs[0]=mxCreateDoubleScalar(ln);
    if (nlhs > 1) plhs[1]=mxCreateDoubleScalar(lr);
    if (nlhs > 2) plhs[2]=mxCreateDoubleScalar(rn);
    if (nlhs > 3) plhs[3]=mxCreateDoubleScalar(rr);
...
{
    if(nrhs<1 || !mxIsChar(prhs[0])) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args","First input must be command string");
    char* cmd_c=mxArrayToString(prhs[0]); if(!cmd_c) mexErrMsgIdAndTxt("snn_regress_gpu_mex:args","Could not parse command"); std::string cmd(cmd_c); mxFree(cmd_c);
    ...
    if(cmd=="predict_gpu"){cmdPredictGpu(nlhs,plhs,nrhs,prhs); return;}
    ...
    mexErrMsgIdAndTxt("snn_regress_gpu_mex:args","Unknown command '%s'",cmd.c_str());
}
