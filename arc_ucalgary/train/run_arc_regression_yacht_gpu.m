function result = run_arc_regression_yacht_gpu()
%RUN_ARC_REGRESSION_YACHT_GPU ARC GPU wrapper generated from Regression/train/GPU_implementation_regression_yacht.mlx.
...
arc_start_diary(repo_root, 'run_arc_regression_yacht_gpu');
arc_print_environment('run_arc_regression_yacht_gpu');

% GPU_implementation_regression_yacht.mlx
% Train the yacht primary-decoder regression task and save the trained network.
...
opts.epochs = arc_epoch_override('SNN_EPOCHS', 100000);
opts.batch_size = 32;
...
model_file = fullfile(model_dir, sprintf('regression_yacht_gpu_primary_seed%03d.mat', opts.seed));
opts = arc_configure_checkpoint(opts, model_file);

arc_compile_required_mex(repo_root, 'regression');
arc_print_training_start('run_arc_regression_yacht_gpu', opts, model_file);
...
if ~(isscalar(value) && isfinite(value) && value >= 1 && value == floor(value))
    error('run_arc_regression_yacht_gpu:badEpochOverride', ...
