function result = run_arc_dynamics_sprotts_gpu()
%RUN_ARC_DYNAMICS_SPROTTS_GPU ARC GPU wrapper generated from dynamical_systems/train/GPU_implementation_dynamical_systems_sprotts.mlx.
...
arc_start_diary(repo_root, 'run_arc_dynamics_sprotts_gpu');
arc_print_environment('run_arc_dynamics_sprotts_gpu');

% GPU_implementation_dynamical_systems_sprotts.mlx
% Train the sprotts primary-decoder dynamical-system task and save the trained network.
...
% N_hidden is the number of spiking neurons; ARC publication runs use 32k.
...
model_file = fullfile(model_dir, sprintf('dynamical_systems_sprotts_gpu_primary_seed%03d.mat', opts.seed));
opts = arc_configure_checkpoint(opts, model_file);

arc_compile_required_mex(repo_root, 'dynamical_systems');
arc_print_training_start('run_arc_dynamics_sprotts_gpu', opts, model_file);
