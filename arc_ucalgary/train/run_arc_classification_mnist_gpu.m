function result = run_arc_classification_mnist_gpu()
%RUN_ARC_CLASSIFICATION_MNIST_GPU ARC GPU wrapper generated from Classification/train/GPU_implementation_classification_MNIST.mlx.
...
arc_start_diary(repo_root, 'run_arc_classification_mnist_gpu');
arc_print_environment('run_arc_classification_mnist_gpu');

% GPU_implementation_classification_MNIST.mlx
% Train the MNIST primary-decoder classification task and save the trained network.
...
model_file = fullfile(model_dir, sprintf('classification_MNIST_gpu_primary_seed%03d.mat', opts.seed));
opts = arc_configure_checkpoint(opts, model_file);

arc_compile_required_mex(repo_root, 'classification');
arc_print_training_start('run_arc_classification_mnist_gpu', opts, model_file);
