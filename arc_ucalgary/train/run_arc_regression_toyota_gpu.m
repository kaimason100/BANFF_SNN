function result = run_arc_regression_toyota_gpu()
%RUN_ARC_REGRESSION_TOYOTA_GPU ARC GPU wrapper generated from Regression/train/GPU_implementation_regression_toyota.mlx.
...
arc_start_diary(repo_root, 'run_arc_regression_toyota_gpu');
arc_print_environment('run_arc_regression_toyota_gpu');

% GPU_implementation_regression_toyota.mlx
% Train the toyota primary-decoder regression task and save the trained network.
...
opts = struct();
opts.task_tag = 'toyota';
opts.dataset_file = 'toyota_dataset.mat';
...
% Publication configuration: all three saved Toyota network seeds were
...
model_file = fullfile(model_dir, sprintf('regression_toyota_gpu_primary_seed%03d.mat', opts.seed));
opts = arc_configure_checkpoint(opts, model_file);

arc_compile_required_mex(repo_root, 'regression');
arc_print_training_start('run_arc_regression_toyota_gpu', opts, model_file);
...
if ~(isscalar(value) && isfinite(value) && value >= 1 && value == floor(value))
    error('run_arc_regression_toyota_gpu:badEpochOverride', ...
