function result = run_arc_regression_abalone_gpu()
%RUN_ARC_REGRESSION_ABALONE_GPU ARC GPU wrapper generated from Regression/train/GPU_implementation_regression_abalone.mlx.
...
arc_start_diary(repo_root, 'run_arc_regression_abalone_gpu');
arc_print_environment('run_arc_regression_abalone_gpu');

% GPU_implementation_regression_abalone.mlx
% Train the abalone primary-decoder regression task and save the trained network.
...
opts = struct();
opts.task_tag = 'abalone';
opts.dataset_file = 'abalone_dataset.mat';
...
% Publication configuration: all three saved Abalone network seeds were
% trained for 25,000 epochs. SNN_EPOCHS remains available as an explicit
% override for intentional alternative runs.
opts.epochs = arc_epoch_override('SNN_EPOCHS', 25000);
opts.batch_size = 32;
...
model_file = fullfile(model_dir, sprintf('regression_abalone_gpu_primary_seed%03d.mat', opts.seed));
opts = arc_configure_checkpoint(opts, model_file);

arc_compile_required_mex(repo_root, 'regression');
arc_print_training_start('run_arc_regression_abalone_gpu', opts, model_file);
...
if ~(isscalar(value) && isfinite(value) && value >= 1 && value == floor(value))
    error('run_arc_regression_abalone_gpu:badEpochOverride', ...
