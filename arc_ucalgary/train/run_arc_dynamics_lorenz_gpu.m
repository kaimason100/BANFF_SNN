function result = run_arc_dynamics_lorenz_gpu()
%RUN_ARC_DYNAMICS_LORENZ_GPU ARC GPU wrapper generated from dynamical_systems/train/GPU_implementation_dynamical_systems_lorenz.mlx.
...
arc_start_diary(repo_root, 'run_arc_dynamics_lorenz_gpu');
arc_print_environment('run_arc_dynamics_lorenz_gpu');

% GPU_implementation_dynamical_systems_lorenz.mlx
% Train the lorenz primary-decoder dynamical-system task and save the trained network.
...
model_file = fullfile(model_dir, sprintf('dynamical_systems_lorenz_gpu_primary_seed%03d.mat', opts.seed));
opts = arc_configure_checkpoint(opts, model_file);

arc_compile_required_mex(repo_root, 'dynamical_systems');
arc_print_training_start('run_arc_dynamics_lorenz_gpu', opts, model_file);
