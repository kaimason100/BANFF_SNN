function result = run_arc_classification_bc_gpu()
%RUN_ARC_CLASSIFICATION_BC_GPU ARC GPU wrapper generated from Classification/train/GPU_implementation_classification_BC.mlx.
...
arc_start_diary(repo_root, 'run_arc_classification_bc_gpu');
arc_print_environment('run_arc_classification_bc_gpu');

% GPU_implementation_classification_BC.mlx
% Train the BC primary-decoder classification task and save the trained network.
...
model_file = fullfile(model_dir, sprintf('classification_BC_gpu_primary_seed%03d.mat', opts.seed));
opts = arc_configure_checkpoint(opts, model_file);

arc_compile_required_mex(repo_root, 'classification');
arc_print_training_start('run_arc_classification_bc_gpu', opts, model_file);
