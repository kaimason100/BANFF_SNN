function result = run_arc_dynamics_vanderpol_full_rank6k_gpu()
%RUN_ARC_DYNAMICS_VANDERPOL_FULL_RANK6K_GPU ARC GPU wrapper generated from dynamical_systems/train/GPU_implementation_dynamical_systems_vanderpol_full_rank6k.mlx.
...
arc_start_diary(repo_root, 'run_arc_dynamics_vanderpol_full_rank6k_gpu');
arc_print_environment('run_arc_dynamics_vanderpol_full_rank6k_gpu');

% GPU_implementation_dynamical_systems_vanderpol_full_rank6k.mlx
% Train the vanderpol full-rank 6k primary-decoder dynamical-system task and save the trained network.
...
% T_sim sets each sampled training snippet length; long_sim_time and burn_in_time define the long true simulation pool used for random contiguous snippets. closed_loop_validate_every controls periodic lightweight GPU closed-loop rollout validation during ARC training.
opts = struct();
...
% N_hidden is the number of spiking neurons; this proof run uses 6k neurons for full-rank recurrence.
...
opts.NET = struct('p_rec', single(0.10), 'variance_correction', true, ...
    'dale', struct('enable', true, 'p_exc', 0.5, 'sign', []));
opts.arch = struct();
opts.arch.recurrent_mode = "full_rank";                  % "low_rank" or "full_rank"
opts.arch.decoder_mode = "signed";                       % "shared" or "signed"
opts.arch.signed_decoder_distribution = "uniform";       % "gaussian"/"normal" or "uniform"
opts.arch.full_rank_p_rec = single(0.10);                 % 10% full-rank recurrent connectivity
opts.arch.full_rank_remove_self_connections = true;
opts.arch.full_rank_storage = "sparse";                   % force sparse storage for 6k full-rank proof runs
...
% epochs can be large for GPU training. CPU scripts intentionally use smaller defaults.
...
model_file = fullfile(model_dir, sprintf('dynamical_systems_vanderpol_full_rank6k_gpu_primary_seed%03d.mat', opts.seed));
opts = arc_configure_checkpoint(opts, model_file);
opts.arc_checkpoint.submit_script = arc_full_rank6k_submit_script(repo_root);

arc_compile_required_mex(repo_root, 'dynamical_systems');
arc_print_training_start('run_arc_dynamics_vanderpol_full_rank6k_gpu', opts, model_file);
...
function submit_script = arc_full_rank6k_submit_script(repo_root)
submit_script = fullfile(repo_root, 'arc_ucalgary', 'submit_arc_full_rank6k_array.slurm');
if exist(submit_script, 'file') ~= 2
    submit_script = fullfile(repo_root, 'submit_arc_full_rank6k_array.slurm');
end
end
