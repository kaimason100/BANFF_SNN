function result = run_arc_regression_yacht_full_rank6k_gpu()
%RUN_ARC_REGRESSION_YACHT_FULL_RANK6K_GPU ARC GPU wrapper generated from Regression/train/GPU_implementation_regression_yacht_full_rank6k.mlx.
...
arc_start_diary(repo_root, 'run_arc_regression_yacht_full_rank6k_gpu');
arc_print_environment('run_arc_regression_yacht_full_rank6k_gpu');

% GPU_implementation_regression_yacht_full_rank6k.mlx
% Train the Yacht Hydrodynamics full-rank 6k primary-decoder regression task and save the trained network.
...
opts.epochs = arc_epoch_override('SNN_EPOCHS', 10000);
opts.batch_size = 32;
...
model_file = fullfile(model_dir, sprintf('regression_yacht_full_rank6k_gpu_primary_seed%03d.mat', opts.seed));
opts = arc_configure_checkpoint(opts, model_file);
opts.arc_checkpoint.submit_script = arc_full_rank6k_submit_script(repo_root);

arc_compile_required_mex(repo_root, 'regression');
arc_print_training_start('run_arc_regression_yacht_full_rank6k_gpu', opts, model_file);
...
% For this task, result.test.metric is Pearson correlation for regression.
result = snn_primary_api('train_static', 'regression', 'gpu', opts);
...
if ~(isscalar(value) && isfinite(value) && value >= 1 && value == floor(value))
    error('run_arc_regression_yacht_full_rank6k_gpu:badEpochOverride', ...
