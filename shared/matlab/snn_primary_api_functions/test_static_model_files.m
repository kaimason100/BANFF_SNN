% test_static_model_files.m
function result = test_static_model_files(domain, backend, opts)
...
for ii = 1:numel(files)
    if exist(files{ii}, 'file') ~= 2
        error('snn_primary_api:modelFileMissing', 'Saved model for seed %g was not found: %s', seeds(ii), files{ii});
    end
    ...
    one_result = test_static(domain, backend, one_opts);
