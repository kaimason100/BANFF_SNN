...
for ii = 1:numel(latest)
    ...
    for jj = 1:numel(A.seeds)
        ...
        entry = struct( ...
            ...
            'task_id', {A.task_id});
        result.seed_results(end + 1, 1) = entry; %#ok<AGROW>
        result.seed_list(end + 1, 1) = entry.init_seed; %#ok<AGROW>
    end
end
...
seed = analysis.seeds(1);
n_hidden = [];
if isfield(seed, 'architecture') && isstruct(seed.architecture) && ...
        isfield(seed.architecture, 'N_hidden')
    n_hidden = seed.architecture.N_hidden;
elseif isfield(seed, 'model') && isstruct(seed.model) && isfield(seed.model, 'N_hidden')
    n_hidden = seed.model.N_hidden;
elseif isfield(seed, 'options') && isstruct(seed.options) && isfield(seed.options, 'N_hidden')
    n_hidden = seed.options.N_hidden;
end
if ~isempty(n_hidden) && isfinite(double(n_hidden(1)))
    tf = any(double(n_hidden(1)) == required_n_hidden(:));
end
end
