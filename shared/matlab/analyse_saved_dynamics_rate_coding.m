...
%   Full CPU baseline spike events, CPU decoder outputs, matched CPU-started
...
result = struct('analysis_kind', 'rate_preserving_within_window_shuffle', ...
    ...
    'description', ['Warmup spike events are replayed unchanged. Scored-interval events are shuffled ', ...
        ...
        'in every window. The unshuffled CPU event replay is verified against its saved CPU decoder ', ...
        'output, and all conditions use the true continuation from that same CPU warmup state.'], ...
...
for aa = 1:numel(analyses)
    ...
    for ss = seed_indices
        ...
        system_result.seed_results(end + 1, 1) = one; %#ok<AGROW>
    end
    require_metric_seed_count(system_result, opts);
    result.systems(end + 1, 1) = system_result; %#ok<AGROW>
...
for ic = 1:n_ic
    ...
    if ~isfinite(replay_error) || replay_error > replay_tolerance
        error('analyse_saved_dynamics_rate_coding:baselineReplayMismatch', ...
            ['Unshuffled CPU event reconstruction differs from the saved CPU decoder output ', ...
    ...
    for ww = 1:numel(conditions)
        ...
        conditions(ww).wd_by_ic(ic) = phase_portrait_wasserstein_distance(double(shuffled), double(truth), get_opt(seed.options, 'wd', struct()));
...
out.baseline_source = 'verified_unshuffled_cpu_event_reconstruction';
out.truth_source = 'cpu_true_continuation_from_same_cpu_warmup_state';
...
if ~isfield(seed.spike_events, 'events_by_ic') || isempty(seed.spike_events.events_by_ic) || ...
        ~isfield(seed, 'model') || ~isstruct(seed.model) || ...
        ...
        ~strcmp(get_field(seed.spike_events, 'recording_backend', ''), 'cpu')
