% Package orientation: Shared loader for Lorenz neuron-count publication figures.

function sweep = load_lorenz_neuron_sweep_publication_data(opts)
%LOAD_LORENZ_NEURON_SWEEP_PUBLICATION_DATA Load newest saved Lorenz test per size.
...
if exist(analysis_dir, 'dir') ~= 7
    error('load_lorenz_neuron_sweep_publication_data:missingDirectory', ...
...
for ii = 1:numel(files)
    ...
    try
        ...
        warning('load_lorenz_neuron_sweep_publication_data:unreadableFile', ...
            'Skipping unreadable saved test-data file %s: %s', files(ii).name, ME.message);
        continue;
    end
    if ~is_lorenz_analysis(A), continue; end
...
if ~isempty(missing)
    error('load_lorenz_neuron_sweep_publication_data:missingCounts', ...
        'No saved Lorenz test analysis was found for N_hidden = [%s].', num2str(missing));
end
...
for ii = 1:numel(neuron_counts)
    ...
    if n < 2 || any(~isfinite(pred(1:n, :)), 'all') || any(~isfinite(truth(1:n, :)), 'all')
        error('load_lorenz_neuron_sweep_publication_data:badExample', ...
            'Lorenz N_hidden=%d has no finite example trajectory for seed index %d.', neuron_counts(ii), example_seed_index);
    end
    ...
    if require_complete_test_ics && n_ic ~= required_test_ic_count
        error('load_lorenz_neuron_sweep_publication_data:incompleteTestIcs', ...
            ['Lorenz N_hidden=%d, network seed %d has %d saved test ICs; this figure requires %d. ', ...
    ...
    conditions(ii).task_id = char(get_field_local(A, 'task_id', 'lorenz'));
...
function tf = is_lorenz_analysis(A)
...
if isempty(position)
    error('load_lorenz_neuron_sweep_publication_data:missingExampleSeed', ...
...
elseif isfield(test, 'pred_norm') && isfield(test, 'true_norm')
    ...
    error('load_lorenz_neuron_sweep_publication_data:missingTrajectory', ...
...
wd = [];
n_ic = min(numel(pred_by_ic), numel(truth_by_ic));
for ic = 1:n_ic
    ...
    for pp = 1:size(pairs, 1)
        pair_wd(pp) = phase_portrait_wasserstein_distance( ...
            pred(1:n, pairs(pp, :)), truth(1:n, pairs(pp, :)), wd_options);
    end
    % One swarm point per test IC: average its three Lorenz phase-plane WDs.
