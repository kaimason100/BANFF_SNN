...
tf = isstruct(A) && isfield(A, 'seeds') && ~isempty(A.seeds);
if ~tf, return; end
seed = A.seeds(1);
model_file = char(string(get_field_local(seed, 'model_file', '')));
% Analyses created on ARC/Windows retain backslash-delimited model paths.
model_file = strrep(model_file, '\', '/');
[~, model_name, model_ext] = fileparts(model_file);
model_file = [model_name model_ext];
% Restrict to the primary low-rank model family so full-rank and SPSA results
% cannot be selected merely because their hidden size happens to match.
pattern = ['^' regexptranslate('escape', task_id) '_gpu_(N[0-9]+_)?primary_seed[0-9]+\.mat$'];
if isempty(regexp(model_file, pattern, 'once')), tf = false; return; end
family = char(string(get_field_local(A, 'task_family', '')));
if strcmp(task_id, 'classification_BC'), tf = strcmp(family, 'classification'); else, tf = strcmp(family, 'regression'); end
end

function n_hidden = analysis_n_hidden(A)
seed = A.seeds(1); n_hidden = NaN;
for candidate = {get_field_local(get_field_local(seed, 'architecture', struct()), 'N_hidden', []), ...
        get_field_local(get_field_local(seed, 'model', struct()), 'N_hidden', []), ...
        get_field_local(get_field_local(seed, 'options', struct()), 'N_hidden', [])}
    value = candidate{1};
    if ~isempty(value) && isfinite(double(value(1))), n_hidden = double(value(1)); return; end
end
end

function [accuracy, rmse, pearson_r, pearson_p, seed_indices] = extract_metrics(seeds, wanted_seed_index)
accuracy = []; rmse = []; pearson_r = []; pearson_p = []; seed_indices = [];
