...
// spsa_classify_time_loop_gpu_mex.cu
//
// Copied classification GPU inner loop for SPSA loss probes with adLIF neurons.
...
// - Legacy train_epoch_gpu commands remain available in this copied source but are not the
//   main SPSA training path.
//
// Command interface from MATLAB:
//
//   spsa_classify_time_loop_gpu_mex('init', ...);
//   spsa_classify_time_loop_gpu_mex('init_optim', ...);
//   spsa_classify_time_loop_gpu_mex('set_data', 'train', X_T, Y_T, true);
//   spsa_classify_time_loop_gpu_mex('set_data', 'val', X_T, Y_T, true);
//   [lossN,lossR,corrN,corrR,count] =
//       spsa_classify_time_loop_gpu_mex('train_epoch_gpu', 'train', order, batch_size, lr_bias);
//   [loss,correct,count] =
//       spsa_classify_time_loop_gpu_mex('validate_primary', 'val', batch_size);
//
// Build:
