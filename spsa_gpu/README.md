...
The dedicated local SPSA test scripts follow recorded `source_file` links and
test the terminal continuation model for each requested seed. The selected
result contains one cumulative history; training plots mark every continuation
boundary with a labelled dashed line and the resolver prints total epochs and
phase boundaries. When several branches descend from one source model, the
resolver selects the terminal branch with the longest cumulative history and
issues a warning listing the alternatives. A single linear continuation chain
per seed remains the clearest and recommended workflow.
