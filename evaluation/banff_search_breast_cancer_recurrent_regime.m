...
for candidateIndex = 1:height(candidates)
    for seedIndex = 1:numel(seeds)
        ...
        try
            ...
            runs.NetToGrossEncoderRms(row) = diagnostics.net_to_gross_encoder_rms;
...
if settings.reuse_completed_models && exist(cfg.model_file, 'file') == 2
    ...
    if isfield(loaded, 'result') && loaded.result.complete && ...
        ...
        banff_provenance("assert_training_compatible", ...
            loaded.result.provenance, loaded.result.config);
...
diagnostic.net_to_gross_encoder_rms = ...
    double(currents.aggregate.net_to_gross_encoder_rms);
...
n = numel(candidates);
summary = table('Size', [n, 23], ...
    'VariableTypes', [repmat({'double'}, 1, 21), {'logical','double'}], ...
    'VariableNames', {'Candidate','EncoderGain','SharedRecurrentDecoderGain', ...
    ...
    'MeanNetToGrossEncoderRms','MeanNetToGrossRecurrentRms', ...
    'MeanDecoderContributionRms', ...
...
for index = 1:n
    ...
    summary.MeanNetToGrossEncoderRms(index) = ...
        mean_finite(runs.NetToGrossEncoderRms(rows));
...
names = {'Stage','Candidate','Seed','HiddenNeurons','EpochBudget', ...
    ...
    'GrossEncoderRmsMv','GrossRecurrentRmsMv', ...
...
types = [{'string'}, repmat({'double'}, 1, 7), {'logical'}, ...
    repmat({'double'}, 1, 17), {'logical','logical','string','string'}];
...
nexttile;
scatter(summary.MeanGrossEncoderRmsMv, ...
    summary.MeanGrossRecurrentRmsMv, 75, ...
    summary.SharedRecurrentDecoderGain, 'filled');
hold on;
limit = max([summary.MeanGrossEncoderRmsMv; ...
    summary.MeanGrossRecurrentRmsMv], [], 'omitnan');
if isempty(limit) || ~isfinite(limit) || limit <= 0, limit = 1; end
plot([0 limit], [0 limit], 'k--');
hold off; axis square; grid on; colorbar;
