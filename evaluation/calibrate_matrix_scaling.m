...
achieved = table(actual_encoder_rms_mV,actual_recurrent_rms_mV, ...
    ...
    final_probe.gross_encoder_rms_mV, ...
    final_probe.gross_recurrent_rms_mV*recommended_recurrent_gain, ...
    final_probe.encoder_net_to_gross_rms_ratio, ...
...
nexttile;
bar([final_probe.gross_encoder_rms_mV ...
    final_probe.gross_recurrent_rms_mV*recommended_recurrent_gain]);
set(gca,'XTickLabel',{'Encoder gross','Recurrent gross'});
ylabel('Gross afferent RMS magnitude (mV)'); grid on;
...
gross_encoder_square = gpuArray.zeros(1,1,'single');
...
for first = 1:double(cfg.batch_size):size(X,2)
    ...
    gross_input_current = abs(P.W_in)*(P.inputScale.* ...
        abs(gpuArray(single(X(:,indices)))));
    ...
    for step = 1:P.presentationSteps
        ...
        gross_encoder_square = gross_encoder_square+ ...
            sum(gross_input_current.*gross_input_current,'all');
...
probe = finish_probe(encoder_square,gross_encoder_square,unit_recurrent_square, ...
...
for step = 1:transition_count
    ...
    gross_input_current = abs(P.W_in)*(P.inputScale.* ...
        abs(gpuArray(single(sequence(:,step)))));
    [unit_recurrent,gross_recurrent] = ...
        unit_recurrent_components(P,unitRecurrent,state.r);
...
function probe = finish_probe(encoderSquare,grossEncoderSquare, ...
        recurrentSquare,grossSquare, ...
...
probe.encoder_net_to_gross_rms_ratio = probe.encoder_rms_mV/ ...
    max(probe.gross_encoder_rms_mV,realmin);
end
